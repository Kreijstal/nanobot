...
class BusWebSocketServer:
    ...
    async def start(self) -> None:
        ...
        # Subscribe to hook events for thinking, tool calls, etc.
        hook_manager.register("agent.thinking", self._handle_thinking)
        hook_manager.register("agent.tool.execute", self._handle_tool_event)
        hook_manager.register("agent.job.created", self._handle_job_event)
    ...
    async def _handle_thinking(self, **kwargs) -> None:
        """Handle agent.thinking event, broadcast to clients."""
        if not self._clients:
            return
    ...
    async def _handle_tool_event(self, **kwargs) -> None:
        """Handle agent.tool.execute event, broadcast to clients."""
        if not self._clients:
            return
    ...
    async def _handle_job_event(self, **kwargs) -> None:
        """Handle agent.job.created event, broadcast to clients."""
        if not self._clients:
            return
