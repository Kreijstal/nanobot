...
from nanobot.providers.custom.opencode import chat_opencode, is_opencode_model
...
class LiteLLMProvider(LLMProvider):
    ...
    async def chat(
        ...
        # Route OpenCode Zen models to custom provider
        if is_opencode_model(original_model, self.api_base):
            opencode_model = original_model.replace("opencode/", "")
            content, raw_tool_calls, finish_reason, usage, thinking = await chat_opencode(
            ...
            # Convert raw dicts to ToolCallRequest objects
            tool_calls = [
                ToolCallRequest(
