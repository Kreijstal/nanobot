...
class QOLManager:
    ...
    def queue_message(self, session_key: str, message: str) -> None:
        ...
        logger.info(f"[QUEUE_DEBUG] Queued message for session {session_key}: {message[:50]}...")
        logger.info(f"[QUEUE_DEBUG] All pending messages: {list(self._pending_messages.keys())}")
