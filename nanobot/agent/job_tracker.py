...
class JobManager:
    ...
    def _job_to_dict(self, job: Job) -> dict:
        ...
        return {
            ...
            "status": job.status,
            "tool_calls": [
                self._tool_call_to_dict(tc) for tc in job.tool_calls
            ],
            "final_response": job.final_response,
            "created_at": job.created_at.isoisoformat() if job.created_at else None,
            "completed_at": job.completed_at.isoisoformat() if job.completed_at else None,
    ...
    def _dict_to_job(self, data: dict) -> Job:
        ...
        job = Job(
            id=data["id"],
            session_key=data.get("session_key") or data.get("sessionKey"),  # handle both formats
    ...
    async def _save(self) -> None:
        ...
        try:
            ...
            data = {
                key: [self._job_to_dict(job) for job in jobs]
                for key, jobs in self._jobs.items()
            }
        ...
        try:
            ...
            for session_key, jobs_data in data.items():
                ...
                for job_data in jobs_data:
                    # Use outer key if session_key is missing/null in job data
                    if not job_data.get("session_key"):
                        job_data["session_key"] = session_key if session_key != "null" else None
    ...
    def has_running_job(self, session_key: str) -> bool:
        """Check if there's a running job for this session."""
        jobs = self._jobs.get(session_key, [])
        return any(job.status in ("running", "pending") for job in jobs)
