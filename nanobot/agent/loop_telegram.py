...
class TelegramAgentLoop(AgentLoop):
    ...
    async def _process_message(
        ...
        # Check for injected pending messages
        inject_messages = msg.metadata.get("inject_messages") if msg.metadata else None
        current_content = msg.content
        if inject_messages and not current_content:
            # Use first injected message as current content
            current_content = inject_messages[0]
            inject_messages = inject_messages[1:]  # Rest will be appended
        ...
        # Inject remaining pending messages as separate user messages
        if inject_messages:
            logger.info(f"Injecting {len(inject_messages)} pending message(s) into context")
            for inject_msg in inject_messages:
                messages.append({"role": "user", "content": inject_msg})
                session.add_message("user", inject_msg)
            if msg.metadata and "inject_messages" in msg.metadata:
                del msg.metadata["inject_messages"]
        ...
        while iteration < self.max_iterations:
            ...
            thinking_content = (
                getattr(response, 'thinking', None) or 
                getattr(response, 'thinking_blocks', None) or
                getattr(response, 'reasoning_content', None)
            )
            if thinking_content:
                thinking_text = thinking_content if isinstance(thinking_content, str) else str(thinking_content)
                logger.info(f"LLM thinking captured ({len(thinking_text)} chars)")
            ...
            if response.has_tool_calls:
                # Send intermediate content (replace timeline if job, else send via bus)
                if response.content and response.content.strip():
                    ...
                    try:
                        ...
                        # Only send via bus if no job (timeline handles it when job exists)
                        if not job_id:
                            intermediate_msg = OutboundMessage(
                                channel=msg.channel,
                                chat_id=msg.chat_id,
                                content=response.content,
                                metadata=make_metadata({"intermediate": True}),
                            )
                ...
                for idx, tool_call in enumerate(response.tool_calls):
                    ...
                    if tool_call.name in ["restart_gateway"]:
                        ...
                        if msg.thread_id:
                            outbound.thread_id = msg.thread_id
                        await self.bus.publish_outbound(outbound)
                    
                    # Inject pending messages after message tool is called
                    if tool_call.name == "message":
                        new_messages = self._get_and_clear_pending_messages(msg.session_key)
                        if new_messages:
                            logger.info(f"Injecting {len(new_messages)} new user message(s) after message tool")
                            ...
                            # Continue processing with new messages - don't break
            else:
        ...
        if message_tool := self.tools.get("message"):
            if isinstance(message_tool, MessageTool) and message_tool._sent_in_turn:
                # Message tool already sent content - just clean up timeline
                if job_id:
                    await job_manager.complete_job(job_id, final_content or "Done.")
                # Check for pending messages and re-queue with metadata to inject all at once
                pending = self._get_and_clear_pending_messages(msg.session_key)
                if pending:
                    logger.info(f"Job ended with {len(pending)} pending message(s), re-queuing with inject")
                    new_metadata = dict(msg.metadata) if msg.metadata else {}
                    new_metadata["inject_messages"] = pending
                    new_msg = InboundMessage(
                        channel=msg.channel,
                        sender_id=msg.sender_id,
                        chat_id=msg.chat_id,
                        content="",  # Content will be from injected messages
                        media=[],
                        metadata=new_metadata,
                    )
                    await self.bus.inbound.put(new_msg)
                return None
        ...
        pending = self._get_and_clear_pending_messages(msg.session_key)
        if pending:
            logger.info(f"Job completed with {len(pending)} pending message(s), re-queuing with inject")
            new_metadata = dict(msg.metadata) if msg.metadata else {}
            new_metadata["inject_messages"] = pending  # Will be injected as separate user messages
            new_msg = InboundMessage(
                channel=msg.channel,
                sender_id=msg.sender_id,
                chat_id=msg.chat_id,
        ...
        # For Telegram, timeline handles the response - return None to prevent duplicate
        # For other channels (web, cli, etc.), return OutboundMessage so it gets sent via bus
