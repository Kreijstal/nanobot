...
class MessageTool(Tool):
    ...
    async def execute(
        ...
        **kwargs: Any
        ...
        # If job_id is present, don't send via bus - the timeline will be replaced instead
        # This prevents duplicate messages (one from bus, one from timeline replacement)
        if job_id:
            if channel == self._default_channel and chat_id == self._default_chat_id:
                self._sent_in_turn = True
            return f"Message queued for job {job_id}"
