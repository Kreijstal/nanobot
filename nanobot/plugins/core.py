...
def _parse_chat_id(chat_id: str | int) -> tuple[int, int | None]:
    """Parse chat_id which may be '123' or '123:456' (chat_id:thread_id)."""
    if isinstance(chat_id, int):
        return chat_id, None
    if ":" in str(chat_id):
        parts = str(chat_id).split(":", 1)
        return int(parts[0]), int(parts[1]) if parts[1] else None
...
class TelegramUIPlugin:
    ...
    async def _on_telegram_init(self, app, channel, **kwargs):
        ...
        logger.info(f"Telegram init: has _agent = {hasattr(self, '_agent')}")

        # Dynamically register agent tools as commands
        if hasattr(self, "_agent"):
            tools = self._agent.tools.get_definitions()
            logger.info(f"Registering {len(tools)} tools as commands")
            for tool_def in tools:
                name = tool_def.get("function", {}).get("name")
                description = tool_def.get("function", {}).get("description", "")[:100]
                if name:
                    logger.debug(f"Registering tool command: /{name}")
                    ...
                    async def tool_command_handler(update, context, tool_name=name):
                        ...
                        # Get thread_id for forum topics
                        thread_id = None
                        if update.message:
    ...
    async def _on_job_created(self, job, channel, chat_id, thread_id=None, **kwargs):
        ...
        """
        logger.info(f"[JOB-CREATED] job={job.id}, channel={channel}, chat_id={chat_id}, thread_id={thread_id}")
        logger.info(f"[JOB-CREATED] has_channel={hasattr(self, '_channel')}, channel_is_none={getattr(self, '_channel', 'NO_ATTR') is None}")
        ...
        if self._is_timeline_disabled():
            logger.info(f"Timeline disabled, skipping creation for job {job.id}")
            return
        
        # Check if _channel is initialized
        if not hasattr(self, '_channel') or self._channel is None:
            logger.warning(f"[JOB-CREATED] Channel not initialized yet, cannot create timeline for job {job.id}")
            return
        
        # Check if this job already has a timeline
        if job.id in self._job_timeline_message:
            logger.debug(f"Timeline already exists for job {job.id}, skipping creation")
            return
        
        # No active job in this chat - create timeline
        ...
        logger.info(f"[TIMELINE-CREATED] job={job.id} message_id={message_id}")
    ...
    async def _on_tool_execute(self, tool_name, tool_args, channel, chat_id, job_id, status, tool_id=None, result=None, thread_id=None, **kwargs):
        ...
        try:
            chat_id_int, parsed_thread = _parse_chat_id(chat_id)
            if parsed_thread and not thread_id:
            ...
            if status == "start":
                ...
                if job_id:
                    ...
                    # Check if this is the message tool - replace timeline with content, create new below
                    if tool_name == "message" and isinstance(tool_args, dict) and "content" in tool_args:
                        ...
                        # Update timeline for all other tools
                        await self._update_timeline(job_id, chat_id_int, thread_id=thread_id)
    ...
    async def _on_response_text(self, channel, chat_id, content, is_final, job_id=None, **kwargs):
        """Replace timeline with response text, create new timeline below for intermediate."""
        ...
        try:
            ...
            # Replace timeline with the response text
            await self._replace_timeline_with_text(
                job_id, chat_id_int, content, is_final=is_final, thread_id=thread_id
            )
        except Exception as e:
    ...
    async def _replace_timeline_with_text(self, job_id: str, chat_id: int, content: str, is_final: bool, thread_id: int | None = None):
        """Replace timeline message with text, then create new timeline below (so timeline stays at bottom).
        ...
        try:
            ...
            self._consumed_message_ids[message_id] = job_id
            logger.info(f"[TIMELINE-CONSUMED] job={job_id}, message_id={message_id}")

            if is_final:
                # Final message - clean up job state
                self._job_view_state.pop(job_id, None)
                self._job_timeline_message.pop(job_id, None)
                self._job_should_recreate_timeline.pop(job_id, None)
                self._timeline_locks.pop(job_id, None)
            else:
                # Create NEW timeline below the message we just sent, preserving view state
                job = await job_manager.get_job(job_id)
                if job:
                    # Preserve current view state
                    current_view_state = self._job_view_state.get(job_id, "timeline")
                    new_message_id = await self._create_timeline_message(chat_id, job, current_view_state, thread_id=thread_id)
                    self._job_timeline_message[job_id] = (chat_id, new_message_id)
                    await job_manager.set_message_id(job_id, str(new_message_id))
                    logger.info(f"[TIMELINE-RECREATED] job={job_id}, new message_id={new_message_id}, view_state={current_view_state}")
            ...
            async def send_split_messages(text: str, parse_mode: str | None = None):
            ...
            try:
                await send_split_messages(content, "Markdown")
            except Exception as e2:
                logger.warning(f"Markdown fallback failed: {e2}, trying plain text")
                try:
                    await send_split_messages(escape_markdown(content))
                except Exception as e3:
                    logger.error(f"All fallbacks failed: {e3}")
                    await self._channel._app.bot.send_message(
                        chat_id=chat_id,
                        text=f"⚠️ Failed to send response: {e3}",
                        message_thread_id=thread_id,
                    )

    async def _delete_timeline(self, job_id: str, chat_id: int):
        """Delete the timeline message (used when message tool sends via bus)."""
        if not job_id or job_id not in self._job_timeline_message:
            return
        ...
        try:
            ...
            self._consumed_message_ids[message_id] = job_id
            del self._job_timeline_message[job_id]
            logger.info(f"[TIMELINE-DELETED] job={job_id}, message_id={message_id}")
        except Exception as e:
    ...
    async def _update_timeline(self, job_id: str, chat_id: int, thread_id: int | None = None):
        ...
        elif job_id in self._job_timeline_message:
            # Job doesn't have message_id set but is in cache - use cache
            chat_id, message_id = self._job_timeline_message[job_id]
            logger.info(f"DEBUG _update_timeline: Using message_id {message_id} from cache for job {job_id}")
        else:
    ...
    async def _on_callback_query(self, update, context):
        ...
        logger.info(f"[PLUGIN-CALLBACK] Callback received: {data} on message_id={message_id} (elapsed: {time.time()-start_time:.3f}s)")
