...
class TelegramChannel(BaseChannel):
    ...
    async def _on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        logger.debug(f"[TELEGRAM] _on_message called: update={update}")
        if not update.message or not update.effective_user:
            logger.debug(f"[TELEGRAM] No message or effective_user in update")
            return
        ...
        # Derive session key: use topic-scoped key for groups with threads, otherwise use chat-scoped key
        if thread_id:
            session_key = f"telegram:{str_chat_id}:{thread_id}"
        else:
            session_key = f"telegram:{str_chat_id}"
        ...
        # Start typing indicator and reaction
